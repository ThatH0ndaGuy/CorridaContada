...
if len(sys.argv)>1:
    ...
    """
        ...
        WRONG_ANSWER = "Wrong answer budy!"
...
while not StopRunning and not ABORT_CODE_EXECUTION:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if GameState==3:
                if event.key == pygame.K_c: # Cheat! Remember to delete this before publishing.
    ...
    if GameState==3:
        ...
        screen.blit(eqscorinfofont_surf, (SCREEN_WIDTH - eqscorinfofont_surf.get_width() - 10, 301))
        screen.blit(EquaScoreTxT, (SCREEN_WIDTH - EquaScoreTxT.get_width() - 10, 286))
        ...
        if eqs>19:
            if ShellLikeTextIndicatorFlashTime%20<10:
                screen.blit(pygame.font.Font("fonts/Octarine.otf", 25).render(BONUS_TIME, True, (255,200,0)), (0,200))
        ...
        if feedback_ticks>0:
            if feedback == -1:
                screen.blit(pygame.font.Font(None, 20).render(WRONG_ANSWER, True, (255, 0, 0)), (0,150)) 
            if feedback == 1:
                screen.blit(pygame.font.Font(None, 20).render(CORRECT_ANSWER, True, (0, 255, 0)), (0,150))
            feedback_ticks -= 1
        else:
            feedback = 0
        ...
        if "/" in USERINPUT:
            ...
            try:
                ...
                screen.blit(pygame.font.Font(None, 20).render(f"{ANSWER_SUGGESTION} {suggest}", True, (255, 255, 255)), (0,60))
            except Exception as e:
                print(e)
                suggest = "-"
    
    if GameState==4 and not ABORT_CODE_EXECUTION:
        screen.blit(loadin,loadin_rect)
        screen.blit(subtitleload,subtitleload_rect)
        pygame.display.flip()
